...
/*
 * Modified by Jon Nall 29-Apr-2005
 */
...
/**
 * <p>
 * This is a 100% Java implementation of the BDD factory. It is based on the C
 * source code for BuDDy. As such, the implementation is very ugly, but it
 * works. Like BuDDy, it uses reference counting scheme for garbage collection.
 * </p>
 *
 * @author John Whaley
 */
    ...
    extends BDDFactory
{
        ...
        extends BDD
    {
        ...
        public double calcWeights()
        {
            ...
            else if (_index == bddtrue)
            {
                topWeight = 1.0;
            }
            else
            {
                topWeight = bddnodes[_index].nodeWeight;
            }
        ...
        /*
         * (non-Javadoc)
         * @see org.sf.javabdd.BDD#nodeWeight()
         */
        public double nodeWeight()
        {
            return bddnodes[_index].nodeWeight;
        }

        /*
         * (non-Javadoc)
         * @see org.sf.javabdd.BDD#edgeWeight(boolean)
         */
        public double edgeWeight(boolean trueEdge)
        {
            return bddnodes[_index].edgeWeight[(trueEdge ? 1 : 0)];
        }
        ...
        /*
         * (non-Javadoc)
         * @see org.sf.javabdd.BDD#relprod(org.sf.javabdd.BDD,
         *      org.sf.javabdd.BDD)
         */
        ...
        public BDD restrictWith(BDD that)
        {
            ...
            //System.out.println("restrictWith("+System.identityHashCode(this)+")
            // "+x+" -> "+a);
            bdd_delref(x);
        ...
        /*
         * (non-Javadoc)
         * @see org.sf.javabdd.BDD#apply(org.sf.javabdd.BDD,
         *      org.sf.javabdd.BDDFactory.BDDOp)
         */
        ...
        /*
         * (non-Javadoc)
         * @see org.sf.javabdd.BDD#applyWith(org.sf.javabdd.BDD,
         *      org.sf.javabdd.BDDFactory.BDDOp)
         */
        public BDD applyWith(BDD that, BDDOp opr)
        {
            ...
            //System.out.println("applyWith("+System.identityHashCode(this)+",
            // "+System.identityHashCode(that)+") "+x+","+y+" -> "+a);
            bdd_delref(x);
            if (this != that) that.free();
            bdd_addref(a);
            this._index = a;
        ...
        /*
         * (non-Javadoc)
         * @see org.sf.javabdd.BDD#applyAll(org.sf.javabdd.BDD,
         *      org.sf.javabdd.BDDFactory.BDDOp, org.sf.javabdd.BDD)
         */
        ...
        /*
         * (non-Javadoc)
         * @see org.sf.javabdd.BDD#applyEx(org.sf.javabdd.BDD,
         *      org.sf.javabdd.BDDFactory.BDDOp, org.sf.javabdd.BDD)
         */
        ...
        /*
         * (non-Javadoc)
         * @see org.sf.javabdd.BDD#applyUni(org.sf.javabdd.BDD,
         *      org.sf.javabdd.BDDFactory.BDDOp, org.sf.javabdd.BDD)
         */
        ...
        /*
         * (non-Javadoc)
         * @see org.sf.javabdd.BDD#satOneSet(org.sf.javabdd.BDD,
         *      org.sf.javabdd.BDD)
         */
        ...
        public BDD replaceWith(BDDPairing pair)
        {
            ...
            //System.out.println("replaceWith("+System.identityHashCode(this)+")
            // "+x+" -> "+y);
            bdd_delref(x);
            bdd_addref(y);
            _index = y;
         ...
         */
        /*
         * protected void finalize() throws Throwable { super.finalize(); if
         * (USE_FINALIZER) { if (false && _index >= 0) { System.out.println("BDD
         * not freed! "+System.identityHashCode(this)); } this.free(); } }
         */
    ...
    static class BddNode
    {
        ...
        final void decRef()
        {
            ...
            if (rc != REF_MASK && rc != 0) refcou_and_level -= REF_INC;
        }
    ...
    static class BddCacheDataI
        extends BddCacheData
    {
        int res;
    }

    static class BddCacheDataD
        extends BddCacheData
    {
        double dres;
    }
    ...
    static String errorstrings[] = { "", "Out of memory", "Unknown variable",
        "Value out of range", "Unknown BDD root dereferenced",
        "bdd_init() called twice", "File operation failed",
        "Incorrect file format", "Variables not in ascending order",
        "User called break", "Mismatch in size of variable sets",
        "Cannot allocate fewer nodes than already in use", "Unknown operator",
        "Illegal variable set", "Bad variable block operation",
        "Trying to decrease the number of variables",
        "Trying to replace with variables already in the bdd",
        "Number of nodes reached user defined maximum",
        "Unknown BDD - was not in node table", "Bad size argument",
        "Mismatch in bitvector size", "Illegal shift-left/right parameter",
        "Division by zero" };
    ...
    void CHECK(int r)
    {
        ...
        else if (r >= 2 && LOW(r) == -1) bdd_error(BDD_ILLBDD);
    }
    ...
    {
        CHECK(root);
        if (root < 2) throw new JavaBDDException(BDD_ILLBDD);
    ...
    {
        CHECK(root);
        if (root < 2) return bdd_error(BDD_ILLBDD);
    ...
    void checkresize()
    {
        if (bddresized != 0) bdd_operator_noderesize();
        bddresized = 0;
    }
    ...
    int bdd_not(int r)
    {
        ...
        again: for (;;)
        {
            try
            {
                ...
                if (firstReorder == 0) bdd_disable_reorder();
                res = not_rec(r);
                if (firstReorder == 0) bdd_enable_reorder();
            }
            catch (BDDException x)
            {
                bdd_checkreorder();
                if (firstReorder-- == 1) continue again;
                res = bddfalse;
    ...
    int not_rec(int r)
    {
        ...
        if (ISZERO(r)) return bddtrue;
        if (ISONE(r)) return bddfalse;

        entry = BddCache_lookupI(applycache, NOTHASH(r));

        if (entry.a == r && entry.c == bddop_not)
        {
            if (CACHESTATS) bddcachestats.opHit++;
            return entry.res;
        }
        if (CACHESTATS) bddcachestats.opMiss++;
    ...
    int replace_rec(int r)
    {
        ...
        res = bdd_correctify(LEVEL(replacepair[LEVEL(r)]), READREF(2),
            READREF(1));
        POPREF(2);
    ...
    int apply_rec(int l, int r)
    {
        ...
        case bddop_xor:
            ...
            break;
        case bddop_nand:
            if (ISZERO(l) || ISZERO(r)) return 1;
            break;
        case bddop_nor:
            if (ISONE(l) || ISONE(r)) return 0;
            break;
        case bddop_imp:
            if (ISZERO(l)) return 1;
            if (ISONE(l)) return r;
            if (ISONE(r)) return 1;
            break;
        }
    ...
    int quant_rec(int r)
    {
        ...
        if (r < 2 || LEVEL(r) > quantlast) return r;
    ...
    int compose_rec(int f, int g)
    {
        ...
        if (LEVEL(f) > composelevel) return f;
    ...
    int veccompose_rec(int f)
    {
        ...
        if (LEVEL(f) > replacelast) return f;
    ...
    int bdd_restrict(int r, int var)
    {
        ...
        again: for (;;)
        {
            if (varset2svartable(var) < 0) return bddfalse;
            try
            {
                INITREF();
    ...
    int bdd_support(int r)
    {
        ...
        /*
         * Update global variables used to speed up bdd_support() - instead of
         * always memsetting support to zero, we use a change counter. - and
         * instead of reading the whole array afterwards, we just look from
         * 'min' to 'max' used BDD variables.
         */
    ...
    void support_rec(int r, int[] support)
    {
        ...
        node = bddnodes[r];
        if (node.isMarked() || node.low == -1) return;

        support[node.getLevel()] = supportID;

        if (node.getLevel() > supportMax) supportMax = node.getLevel();
    ...
    void bdd_default_gbchandler(boolean pre, bddGbcStat s)
    {
        if (!pre)
        {
            System.err.print("Randsolver BDD Garbage collection #" + s.num
                + ": " + s.nodes + " nodes / " + s.freenodes + " free");
            System.err.println(" / " + (float) s.time / (float) 1000 + "s / "
                + (float) s.sumtime / (float) 1000 + "s total");
        }
    }
    ...
    double bdd_pathcount_rec(int r)
    {
        ...
        if (ISONE(r)) return 1.0;

        entry = BddCache_lookupD(countcache, PATHCOUHASH(r));
        if (entry.a == r && entry.c == miscid) return entry.dres;
    ...
    double satcount_rec(int root)
    {
        ...
        if (entry.a == root && entry.c == miscid) return entry.dres;

        node = bddnodes[root];
        size = 0;
        s = 1;

        s *= Math.pow(2.0, (LEVEL(node.low) - node.getLevel() - 1));
        size += s * satcount_rec(node.low);

        s = 1;
        s *= Math.pow(2.0, (LEVEL(node.high) - node.getLevel() - 1));
    ...
    {
        if (root == -1) bdd_error(BDD_BREAK); /* distinctive */
        if (root < 2 || !bddrunning) return root;
        if (root >= bddnodesize) return bdd_error(BDD_ILLBDD);
        if (LOW(root) == -1) return bdd_error(BDD_ILLBDD);
    ...
    int bdd_makenode(int level, int low, int high)
    {
        ...
        if (CACHESTATS) bddcachestats.uniqueAccess++;
        ...
        while (res != 0)
        {
            ...
            {
                if (CACHESTATS) bddcachestats.uniqueHit++;
                return res;
            }

            res = bddnodes[res].next;
            if (CACHESTATS) bddcachestats.uniqueChain++;
        }

        /* No existing node => build one */
        if (CACHESTATS) bddcachestats.uniqueMiss++;
        ...
        if (bddfreepos == 0)
        {
             ...
             * bdd_reorder_ready()) { longjmp(bddexception,1); }
             */
        ...
        /* Calc weights */
        double loWeight = (low == bddfalse ? 0.0 : bddnodes[low].nodeWeight);
        double hiWeight = (high == bddfalse ? 0.0 : bddnodes[high].nodeWeight);
        node.edgeWeight[0] = 0.5 * loWeight;
        node.edgeWeight[1] = 0.5 * hiWeight;
        node.nodeWeight = node.edgeWeight[0] + node.edgeWeight[1];
    ...
    int bdd_noderesize(boolean doRehash)
    {
        ...
        if (bddnodesize >= bddmaxnodesize && bddmaxnodesize > 0) return -1;
        ...
        if (doRehash) for (n = 0; n < oldsize; n++)
            bddnodes[n].hash = 0;
    ...
    protected void initialize(int initnodesize, int cs)
    {
        ...
        if (bddrunning) throw new JavaBDDException(BDD_RUNNING);
    ...
    int firstReorder;
    /*
     * Used instead of local variable in order to avoid compiler warning about
     * 'first' being clobbered by setjmp
     */
    ...
    {
        if (quantvarset != null) free(quantvarset);
    ...
    void bdd_setpair(bddPair pair, int oldvar, int newvar)
    {
        ...
        if (oldvar < 0 || oldvar > bddvarnum - 1) bdd_error(BDD_VAR);
        if (newvar < 0 || newvar > bddvarnum - 1) bdd_error(BDD_VAR);

        bdd_delref(pair.result[bddvar2level[oldvar]]);
        pair.result[bddvar2level[oldvar]] = bdd_ithvar(newvar);
        pair.id = update_pairsid();

        if (bddvar2level[oldvar] > pair.last) pair.last = bddvar2level[oldvar];
    ...
    void bdd_setbddpair(bddPair pair, int oldvar, int newvar)
    {
        ...
        CHECK(newvar);
        if (oldvar < 0 || oldvar >= bddvarnum) bdd_error(BDD_VAR);
        ...
        if (oldlevel > pair.last) pair.last = oldlevel;
    ...
    boolean bdd_reorder_ready()
    {
        if ((bddreordermethod == BDD_REORDER_NONE) || (vartree == null)
            || (bddreordertimes == 0) || (reorderdisabled != 0)) return false;
        return true;
    }
    ...
    void bdd_reorder(int method)
    {
        ...
        if ((top = bddtree_new(-1)) == null) return;
        if (reorder_init() < 0) return;
    ...
    BddTree reorder_win2(BddTree t)
    {
        ...
        while (dis.next != null)
        {
            ...
            else if (first == dis) first = dis.prev;
    ...
    BddTree reorder_win3ite(BddTree t)
    {
        ...
        if (verbose > 1)
            System.out
                .println("Win3ite start: " + reorder_nodenum() + " nodes");
    ...
    BddTree reorder_swapwin3(BddTree dis, BddTree[] first)
    {
        ...
        /* Real win3 swap */{
            ...
            if (pos >= 2) /* C A B -> C B A* */
            {
                blockdown(dis);
                next = dis.prev;
                if (setfirst) first[0] = dis.prev.prev;
            }
    ...
    BddTree reorder_sift_seq(BddTree t, BddTree seq[], int num)
    {
        ...
        for (n = 0; n < num; n++)
        {
            ...
            c2 = clock();
            if (verbose > 1)
                System.out.println(" (" + (float) (c2 - c1) / (float) 1000
                    + " sec)\n");
        }
    ...
    void reorder_sift_bestpos(BddTree blk, int middlePos)
    {
        ...
        if (bddmaxnodesize > 0)
            maxAllowed = MIN(best / 5 + best, bddmaxnodesize
                - bddmaxnodeincrease - 2);
        else
            maxAllowed = best / 5 + best;

        /* Determine initial direction */
        if (blk.pos > middlePos) dirIsUp = false;
    ...
    BddTree reorder_random(BddTree t)
    {
        ...
        for (n = 0; n < 4 * num; n++)
        {
            ...
            if (seq[blk].next != null) blockdown(seq[blk]);
        }
    ...
    BddTree reorder_siftite(BddTree t)
    {
        ...
        do
        {
            if (verbose > 1) System.out.println("Reorder " + (c++) + "\n");
    ...
    void blockdown(BddTree left)
    {
        ...
        while (bddvar2level[rseq[0]] > leftstart)
        {
            ...
            if (bddvar2level[rseq[0]] > leftstart) reorder_varup(rseq[0]);
        }
        ...
        if (right.prev != null) right.prev.next = right;
    ...
    BddTree reorder_win2ite(BddTree t)
    {
        ...
        if (verbose > 1)
            System.out
                .println("Win2ite start: " + reorder_nodenum() + " nodes");
    ...
    int bdd_setminfreenodes(int mf)
    {
        ...
        if (mf < 0 || mf > 100) return bdd_error(BDD_RANGE);
    ...
    int bdd_setmaxincrease(int size)
    {
        ...
        if (size < 0) return bdd_error(BDD_SIZE);
    ...
    int bdd_setcacheratio(int r)
    {
        ...
        if (r <= 0) return bdd_error(BDD_RANGE);
        if (bddnodesize == 0) return old;
    ...
    int bdd_setvarnum(int num)
    {
        ...
        if (num < bddvarnum) return bdd_error(BDD_DECVNUM);
        if (num == bddvarnum) return 0;
        ...
        else
        {
            ...
            System.arraycopy(bddlevel2var, 0, bddlevel2var2, 0,
                bddlevel2var.length);
            bddlevel2var = bddlevel2var2;
            int[] bddvar2level2 = new int[num + 1];
            System.arraycopy(bddvar2level, 0, bddvar2level2, 0,
                bddvar2level.length);
            bddvar2level = bddvar2level2;
        }

        if (bddrefstack != null) free(bddrefstack);
    ...
    {
        int x = ((bdd) b)._index;
        DataOutputStream is = new DataOutputStream(new FileOutputStream(
            filename));
        bdd_save(is, x);
        is.close();
    }
    ...
    void bdd_default_reohandler(int prestate)
    {
        ...
        if (verbose > 0)
        {
            ...
            {
                long c2 = clock();
                System.out.println("End reordering. Went from "
                    + usednum_before + " to " + usednum_after + " nodes ("
    ...
    {
        if (var < 0 || var >= bddvarnum) return bdd_error(BDD_VAR);
        if (bddvar2level[var] == 0) return 0;
    ...
    int reorder_vardown(int var)
    {
        ...
        if (var < 0 || var >= bddvarnum) return bdd_error(BDD_VAR);
        if ((level = bddvar2level[var]) >= bddvarnum - 1) return 0;
        ...
        if (imatrixDepends(iactmtx, var, bddlevel2var[level + 1]))
        {
            ...
            if (l.nodenum < (l.size) / 3 || l.nodenum >= (l.size * 3) / 2
                && l.size < l.maxsize)
            {
        ...
        if (resizedInMakenode) reorder_rehashAll();
    ...
    void reorder_setLevellookup()
    {
        ...
        for (n = 0; n < bddvarnum; n++)
        {
            ...
            levels[n].size = Math.min(levels[n].maxsize,
                (levels[n].nodenum * 5) / 4);
        ...
        for (n = 0; n < size0; n++)
        {
            ...
            while (r != 0)
            {
                ...
                else
                {
                    ...
                    toBeProcessed = r;
                    if (SWAPCOUNT) bddcachestats.swapCount++;
    ...
    void reorder_swapResize(int toBeProcessed, int var0)
    {
        ...
        while (toBeProcessed != 0)
        {
             ...
             * node.low/high nodes to come alive again, so deref. of the childs
             * is delayed until the local GBC.
             */
    ...
    int mark_roots()
    {
        ...
        for (n = 2, extrootsize = 0; n < bddnodesize; n++)
        {
             ...
             * macro here.
             */
            bddnodes[n].setLevelAndMarked(bddlevel2var[bddnodes[n]
                .getLevelAndMarked()]);
    ...
    void addref_rec(int r, boolean[] dep)
    {
        ...
        else
        {
             ...
             */
            for (n = 0; n < bddvarnum; n++)
                dep[n] |= imatrixDepends(iactmtx, VARr(r) & ~BddNode.MARK_MASK,
                    n);
        }
    ...
    void reorder_done()
    {
        ...
        for (n = 2; n < bddnodesize; n++)
        {
             ...
             * LEVEL macro here.
             */
            bddnodes[n].setLevelAndMarked(bddvar2level[bddnodes[n]
                .getLevelAndMarked()]);
        }
    ...
    int bdd_swapvar(int v1, int v2)
    {
        ...
        if (vartree != null) return bdd_error(BDD_VARBLK);
    ...
    String readNext(DataInput ifile)
        throws IOException
    {
        while (tokenizer == null || !tokenizer.hasMoreTokens())
        {
            String s = ifile.readLine();
            if (s == null) bdd_error(BDD_FORMAT);
            tokenizer = new StringTokenizer(s);
        }
        return tokenizer.nextToken();
    }

    int bdd_load(DataInput ifile)
        throws IOException
    {
        int n, vnum, tmproot;
        int root;
        ...
        if (vnum > bddvarnum) bdd_setvarnum(vnum);
    ...
    int bdd_loaddata(DataInput ifile)
        throws IOException
    {
        ...
        for (n = 0; n < lh_nodenum; n++)
        {
            ...
            if (low >= 2) low = loadhash_get(low);
            if (high >= 2) high = loadhash_get(high);

            if (low < 0 || high < 0 || var < 0) return bdd_error(BDD_FORMAT);
    ...
    void bdd_save(DataOutput out, int r)
        throws IOException
    {
        int[] n = new int[1];
    ...
    void bdd_save_rec(DataOutput out, int root)
        throws IOException
    {
    ...
    int bdd_addvarblock(int[] v, boolean fixed)
    {
        ...
        if (v.length < 1) return bdd_error(BDD_VARBLK);

        first = last = v[0];

        for (int n = 0; n < v.length; n++)
        {
            if (v[n] < first) first = v[n];
            if (v[n] > last) last = v[n];
        }
        ...
        int id)
    {
        ...
        if (t == null)
        {
            if ((t = bddtree_new(id)) == null) return null;
            t.first = first;
            t.fixed = fixed;
        ...
        /* Check for identity */
        if (first == t.first && last == t.last) return t;
        ...
        {
            t.nextlevel = bddtree_addrange_rec(t.nextlevel, null, first, last,
                fixed, id);
            return t;
        }
        ...
        if (first <= t.first)
        {
            ...
            {
                /* Partial cover ->error */
                if (last >= dis.first && last < dis.last) return null;
                ...
                if (dis.next == null || last < dis.next.first)
                {
                    ...
                    tnew.next = dis.next;
                    tnew.prev = t.prev;
                    if (dis.next != null) dis.next.prev = tnew;
                    dis.next = null;
                    t.prev = null;
                    return tnew;
                }
    ...
    void update_seq(BddTree t)
    {
        ...
        for (n = t.first; n <= t.last; n++)
            if (bddvar2level[n] < bddvar2level[low]) low = n;
    ...
    void bdd_fprintstat(PrintStream out)
    {
        ...
        out.println("=> Hit rate =   "
            + ((s.uniqueHit + s.uniqueMiss > 0) ? (s.uniqueHit)
                / ((float) s.uniqueHit + s.uniqueMiss) : 0));
        out.println("Operator Hits:  " + s.opHit);
        out.println("Operator Miss:  " + s.opMiss);
        out
            .println("=> Hit rate =   "
                + ((s.opHit + s.opMiss > 0) ? (s.opHit) / (s.opHit + s.opMiss)
                    : 0));
    ...
    static boolean isWitness(int witness, int src)
    {
        ...
        for (i = bitNum; i >= 0; --i)
        {
            ...
            if (d == 1 && x != 1 && x != src - 1) return true;

            if (BitIsSet(src - 1, i)) d = u64_mulmod(d, witness, src);
        }
    ...
    static boolean isMillerRabinPrime(int src)
    {
        ...
        for (n = 0; n < CHECKTIMES; ++n)
        {
            ...
            if (isWitness(witness, src)) return false;
        }
    ...
    {
        if (hasEasyFactors(src)) return false;
