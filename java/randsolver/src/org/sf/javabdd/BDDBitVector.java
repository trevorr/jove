    ...
    implements BDDExpr
{
    ...
    public void setSigned(boolean signBit)
    {
        isSigned = signBit;
    }
    ...
    public boolean containsBDD(BDD bdd)
    {
        for (int i = 0; i < bitvec.length; ++i)
        {
            if (bitvec[i].equals(bdd))
            {
                return true;
            }
        }
    ...
    {
        return isSigned() && (bitvec[bitvec.length - 1].isOne());
    }
    ...
    public BDDBitVector coerce(int bitnum)
    {
        ...
        BDDFactory bdd = getFactory();
        BDD lastBit = bdd.zero();
        BDD[] newBitVec = new BDD[bitnum];
        for (int i = 0; i < bitnum; i++)
        {
            if (i < bitvec.length)
            {
                newBitVec[i] = bitvec[i].id();
                lastBit = bitvec[i].id();
            }
            else if (isSigned)
            {
    ...
    {
        if (bitvec.length != that.bitvec.length) throw new BDDException();
    ...
    {
        if (this.bitvec.length != r.bitvec.length) throw new BDDException();
        ...
        int step)
    {
        ...
        BDDBitVector newRemainder = tmp.shl(1,
            result.bitvec[divisor.bitvec.length - 1]);

        if (step > 1) div_rec(divisor, newRemainder, newResult, step - 1);
    ...
    public BDDBitVector shl(int pos, BDD c)
    {
        ...
        if (minnum < 0) throw new BDDException();
    ...
    public BDDBitVector shr(int pos, BDD c)
    {
        ...
        if (maxnum < 0) throw new BDDException();
    ...
    {
        if (c <= 0L) throw new BDDException();
        BDDFactory bdd = getFactory();
    ...
    public void setBit(int n, BDD bdd)
    {
        bitvec[n] = bdd;
    }
