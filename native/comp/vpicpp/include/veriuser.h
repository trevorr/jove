...
/*------- structure for use with tf_mdanodeinfo() to get mda
node information ------  */
...
/* Make sure the 64-bit platforms are correctly prototyped. */
#if !defined(_LP64)
#if defined(__sparcv9) || defined(__LP64__) || defined(__ia64)
#define _LP64 1
# endif
#endif
...
EXTERN void         io_mcdprintf PROTO_PARAMS((int mcd, const char *format, ...));
EXTERN void         io_printf PROTO_PARAMS((char *format, ...));
EXTERN char        *mc_scan_plusargs PROTO_PARAMS((const char *plusarg));
EXTERN int          tf_add_long PROTO_PARAMS((int *aof_lowtime1, int *aof_hightime1, int lowtime2, int hightime2));
...
EXTERN LP64BIT      tf_getp PROTO_PARAMS((int pnum));
...
EXTERN char        *tf_gettflist PROTO_PARAMS((void));
...
EXTERN LP64BIT      tf_igetp PROTO_PARAMS((int pnum, char *inst));
...
EXTERN int          tf_isetworkarea PROTO_PARAMS((char *workarea, char *inst));
...
EXTERN int          tf_subtract_long PROTO_PARAMS((int *aof_lowtime1, int *aof_hightime1, int lowtime2, int hightime2));
...
EXTERN char        *tf_getroutine PROTO_PARAMS((void));
EXTERN char        *tf_igetroutine PROTO_PARAMS((char *inst));

/* routines added for MDA support  */
EXTERN p_tfmdanodeinfo tf_mdanodeinfo PROTO_PARAMS((int pnum, p_tfmdanodeinfo pinfo));
EXTERN p_tfmdanodeinfo tf_imdanodeinfo PROTO_PARAMS((int pnum, p_tfmdanodeinfo pinfo, char *inst));
