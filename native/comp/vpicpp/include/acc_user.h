...
/* bool must be defined locally as int, but must be undefined later for C++ */
#ifndef bool
#define ACC_BOOL_DEFINED
#define bool   int
#define true   1
#define false  0
#endif
...
extern DLL_IMPORT bool acc_error_flag;
...
EXTERN bool    acc_append_pulsere PROTO_PARAMS((handle object, double val1r, double val1x, ...));
EXTERN void    acc_close PROTO_PARAMS((void));
EXTERN handle *acc_collect PROTO_PARAMS((handle (*p_next_routine)(), handle scope_object, int *aof_count));
...
EXTERN int     acc_count PROTO_PARAMS((handle (*next_func)(), handle object_handle));
...
EXTERN int     acc_fetch_location PROTO_PARAMS((p_location location_p, handle object));
...
EXTERN bool    acc_fetch_pulsere PROTO_PARAMS((handle path_p, double *val1r, double *val1e, ...));
...
EXTERN void    acc_fetch_timescale_info PROTO_PARAMS((handle obj, p_timescale_info aof_timescale_info));
EXTERN int     acc_fetch_type PROTO_PARAMS((handle object_handle));
EXTERN char   *acc_fetch_type_str PROTO_PARAMS((int type));
EXTERN char   *acc_fetch_value PROTO_PARAMS((handle object_handle, char *format_str, p_acc_value acc_value_p));
EXTERN void    acc_free PROTO_PARAMS((handle *array_ptr));
EXTERN handle  acc_handle_by_name PROTO_PARAMS((char *inst_name, handle scope_p));
...
EXTERN handle  acc_handle_modpath PROTO_PARAMS((handle mod_p, char *pathin_name, char *pathout_name, ...));
...
EXTERN handle  acc_handle_tchk PROTO_PARAMS((handle mod_p, int tchk_type, char *arg1_conn_name, int arg1_edgetype, ...));
EXTERN handle  acc_handle_tchkarg1 PROTO_PARAMS((handle tchk));
EXTERN handle  acc_handle_tchkarg2 PROTO_PARAMS((handle tchk));
EXTERN handle  acc_handle_terminal PROTO_PARAMS((handle gate_handle, int terminal_index));
...
EXTERN handle  acc_next PROTO_PARAMS((int *type_list, handle h_scope, handle h_object));
...
EXTERN handle  acc_next_parameter PROTO_PARAMS((handle module_p, handle param));
...
EXTERN handle  acc_next_primitive PROTO_PARAMS((handle mod_handle, handle prim));
EXTERN handle  acc_next_scope PROTO_PARAMS((handle ref_scope_p, handle scope));
EXTERN handle  acc_next_specparam PROTO_PARAMS((handle module_p, handle sparam));
...
EXTERN bool    acc_replace_pulsere PROTO_PARAMS((handle object, double val1r, double val1x, ...));
EXTERN void    acc_reset_buffer PROTO_PARAMS((void));
EXTERN bool    acc_set_interactive_scope PROTO_PARAMS((handle scope, int callback_flag));
EXTERN bool    acc_set_pulsere PROTO_PARAMS((handle path_p, double val1r, double val1e));
EXTERN char   *acc_set_scope PROTO_PARAMS((handle object, ...));
EXTERN int     acc_set_value PROTO_PARAMS((handle obj, p_setval_value setval_p, p_setval_delay delay_p));
EXTERN void    acc_vcl_add PROTO_PARAMS((handle object_p, int (*consumer)(), char *user_data, int vcl_flags));
EXTERN void    acc_vcl_delete PROTO_PARAMS((handle object_p, int (*consumer)(), char *user_data, int vcl_flags));
...
#if defined(__cplusplus) && defined(ACC_BOOL_DEFINED)
#undef ACC_BOOL_DEFINED
#undef bool
#undef true
#undef false
#endif
